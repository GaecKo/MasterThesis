...
for label, entry in series.items():
    ...
    ax.plot(df["achieved_rps"], df["p50"],
            ...
            markeredgewidth=1.3, zorder=ZORDER.get(label, 3))
...
CPU_SMOOTH = 8   # rolling median window for B — 4 CPUs only
...
for label, entry in series.items():
    ...
    # Apply rolling median smoothing only to B — 4 CPUs to suppress spikes
    if label == "B — 4 CPUs":
        cpu_values = df["cpu_pct"].rolling(CPU_SMOOTH, center=True, min_periods=1).median() / 100
    else:
        cpu_values = df["cpu_pct"] / 100

    ax2.plot(df["achieved_rps"], cpu_values,
             color=color, linewidth=1.0, linestyle=ls,
             alpha=0.35, zorder=2)

    # RAM annotation at peak RPS
    peak_row = df.loc[df["achieved_rps"].idxmax()]
    if not np.isnan(peak_row["ram_mib"]):
        ax2.annotate(
            f"{peak_row['ram_mib']:.0f} MiB",
            xy=(peak_row["achieved_rps"], cpu_values.iloc[df["achieved_rps"].idxmax()]),
